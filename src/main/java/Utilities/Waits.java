...
public class Waits
{
    ...
    public static void setImplicitWait(int time){
        BaseDriver.driver.manage().timeouts().implicitlyWait(time, TimeUnit.SECONDS);
    }


    public static void waitUntilElementLocated(int time,WebElement element){
        WebDriverWait wait=new WebDriverWait(BaseDriver.driver,time);
    ...
    public static synchronized Object execJavascript(String script, Object... args) {
        JavascriptExecutor scriptExe = ((JavascriptExecutor)BaseDriver.driver);
    ...
    public static void waitUntil(BooleanSupplier condition, int seconds) {
        new WebDriverWait(BaseDriver.driver, seconds).until((WebDriver driver) -> condition.getAsBoolean());
    }
